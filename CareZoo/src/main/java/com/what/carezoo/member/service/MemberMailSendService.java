...
public class MemberMailSendService {
	...
	// 이메일 난수 만드는 메서드
		...
		public boolean mailSendacceptHome(HomeSitterReservation hsr, HomeSitter hs, Customer c, HttpServletRequest request) {
			...
			+"</label>&nbsp;&nbsp;"
			+"<p><label>결제금액: "+hsr.getHsr_totalprice()+"원"
			+ " 예약이 <label style='color:red'>수락</label>되었습니다."
			+ "마이페이지에서 결제를 해주세요"
			+"<p>감사합니다 (_ _)";
			try {
				mail.setSubject("예약수락메일입니다.", "utf-8");
				mail.setText(htmlStr, "utf-8", "html");
				mail.addRecipient(RecipientType.TO, new InternetAddress(c.getC_email()));
				mailSender.send(mail);
				return true;
		...
		// 회원탈퇴 발송 이메일
		public boolean mailSendWithMember(String c_email,String c_name, HttpServletRequest request) {
			
			MimeMessage mail = mailSender.createMimeMessage();
			String htmlStr = "<h2>안녕하세요 MS :p 맡겨쥬 입니다!</h2><br><br>" 
					+ "<h3>" + c_name + "님</h3>" + "<br>부적절한 홈페이지 활동으로 인하여 본 홈페이지에서 <span style='color:red; font-size:15;'>강제탈퇴</span> 되었음을 알려드립니다." 
