...
<body class="container">
	...
	<div class="container" id="visit">
		...
		<form action="petList" method="post">
				...
				<div>
					...
					<div><input type="hidden" name="c_num" value="${c_num}">
						...
						${pet.p_name}(${pet.p_weight},${pet.p_birth}) <input type="button"
						name="p_num" onclick="petDeleteOpen(${pet.p_num},${c_num})"
						class="btn btn-cancle" value="x"></div>
				</div>
			</c:forEach>
			<div>
				<input type="submit" class="btn btn-submit" value="다음" onclick="return test_checkbox()">
				<!-- return: 클릭했을 때 함수호출
		...
		<button
			onclick="location.href='${contextPath}/visit/apply?c_num=${c_num}'" class="btn btn-add">새로운
			반려견 등록</button>
	</div>
